...
- сообщение REPLY_USER может использоваться для восстановления темы
  сообщения CURRENT_USER;
- принадлежность сообщения REPLY_USER другому человеку
  не запрещает использовать его для определения темы,
  но не позволяет переносить намерение REPLY_USER на CURRENT_USER;
...
- диагностический или уточняющий вопрос,
  заданный для помощи REPLY_USER,
  должен получить intent_score не выше 20;
- реакция, комментарий или обсуждение ситуации REPLY_USER
  без собственной задачи CURRENT_USER
  должны получить intent_score от 21 до 49;
- если принадлежность задачи CURRENT_USER не подтверждена,
